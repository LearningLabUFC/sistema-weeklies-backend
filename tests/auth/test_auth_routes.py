...
def test_forgot_password_rate_limit_ip(client):
    ...
    for i in range(limite_ip):
        res = client.post(
            "/auth/forgot-password", json={"email": f"ip_test{i}@teste.com"}
        )
        assert res.status_code == 200
...
def test_verify_code_sucesso(client, monkeypatch):
    ...
    monkeypatch.setattr(auth_service, "gerar_codigo_otp", lambda: "123456")
...
def test_verify_code_bruteforce(client, monkeypatch):
    ...
    for _ in range(4):
        res = client.post(
            "/auth/verify-code",
            json={"email": "integracao@teste.com", "codigo": "errado"},
        )
        assert res.status_code == 401
...
def test_refresh_token_sucesso(client, usuario_ativo_logado):
    ...
    res = client.post(
        "/auth/refresh", json={"token_atualizacao": tokens["token_atualizacao"]}
    )
    assert res.status_code == 200
...
def test_logout_sucesso(client, usuario_ativo_logado):
    ...
    res_change_pwd = client.put(
        ...
        json={"senha_atual": "SenhaForte123!", "nova_senha": "NovaSenhaSegura1@"},
        headers=headers,
    )
    ...
    res_refresh = client.post(
        "/auth/refresh", json={"token_atualizacao": token_atualizacao}
    )
...
def test_change_password_incorreta(client, usuario_ativo_logado):
    ...
    res = client.put(
        "/auth/change-password",
        json={"senha_atual": "SenhaErrada!!!", "nova_senha": "NovaSenhaSegura1@"},
        headers=headers,
    )
