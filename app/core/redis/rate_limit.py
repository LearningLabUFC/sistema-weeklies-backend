...
async def verificar_rate_limit_email(email: str) -> bool:
    ...
    return not (
        contagem is not None and int(contagem) >= settings.FORGOT_PASSWORD_MAX_REQUESTS
    )
...
async def verificar_rate_limit_ip(ip: str) -> bool:
    ...
    contagem = await r.get(chave)
    return not (
        contagem is not None
        and int(contagem) >= settings.FORGOT_PASSWORD_IP_MAX_REQUESTS
    )
